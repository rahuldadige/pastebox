...
// Helper to safely calculate days until expiry
const getExpiryText = (expiresAt) => {
  ...
  try {
    ...
    return days <= 0 ? "Expired" : `Expires in ${days} days`;
  } catch {
    return "Unknown";
  }
};

// Helper to safely format upload time
const getUploadedText = (createdAt) => {
  if (!createdAt) return "Unknown";
  try {
    const date = new Date(createdAt);
    if (isNaN(date.getTime())) return "Unknown";
    return `Uploaded ${formatDistanceToNowStrict(date, { addSuffix: true })}`;
  } catch {
    return "Unknown";
  }
};
...
const isFileExpired = (expiresAt) => {
  ...
  try {
    ...
    return differenceInDays(expiryDate, new Date()) <= 0;
  } catch {
    return false;
  }
};
...
const GuestFilePreview = ({ guestFiles }) => {
  ...
  const filteredFiles = (files || []).filter((file) => {
    ...
    if (filterStatus && file.expiresAt) {
      try {
        const expiryDate = new Date(file.expiresAt);
        if (!isNaN(expiryDate.getTime())) {
          statusMatch = filterStatus === "expired"
            ? differenceInDays(expiryDate, new Date()) <= 0
            : differenceInDays(expiryDate, new Date()) > 0;
        }
      } catch (e) {
        statusMatch = true;
    ...
    <div className="flex flex-col mt-6">
      ...
      <div className="flex flex-col lg:flex-row gap-4 w-full lg:items-center mb-4">
        ...
        >
          ...
          {[...new Set((files || []).map((f) => f?.type).filter(Boolean))].map((type) => (
              ...
              <table className="min-w-full divide-y divide-[var(--border-color)] text-[var(--text-color)]">
                ...
                <tbody className="bg-[var(--bg-color)] divide-y divide-[var(--border-color)]">
                  {paginatedFiles?.map((file, index) => {
                    ...
                    return (
                      <React.Fragment key={file.id || file._id || index}>
                        {/* Desktop Row */}
                        <tr
                          className="hover:bg-[var(--hover-bg-color)] hidden md:table-row"
                        >
                          <td className="px-6 py-4 text-sm">
                            {sortFileName(file.name || 'Unknown')}
                          </td>
                          ...
                          <td className="px-6 py-4 text-sm">
                            <span
                              className={`font-medium ${
                                file.status === "active" || !file.status
                                  ? "text-green-500"
                                  : "text-red-500"
                              }`}
                            >
                          ...
                          <td className="px-6 py-4 text-sm text-red-500">
                            {getExpiryText(file.expiresAt)}
                          </td>
                          <td className="px-6 py-4 text-sm text-gray-400">
                            {getUploadedText(file.createdAt)}
                          </td>
                        </tr>
                          ...
                          <td className="block px-4 py-4">
                            <div className="mb-2">
                              ...
                              <div className="text-xs text-gray-400">
                                {file.type || 'Unknown'} | {formattedSize}
                              </div>
                            </div>
